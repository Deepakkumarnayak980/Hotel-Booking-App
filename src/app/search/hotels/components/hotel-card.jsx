...
const HotelImages = ({ photos = [] }) => {
  ...
  if (photos.length === 0) {
    return (
      <div className="w-60 h-[138px] bg-gray-100 flex items-center justify-center rounded-sm text-xs text-gray-500">
        No Image
      </div>
    );
  }

  const handleHover = (index) => setActiveImageIndex(index);
      ...
      <div className="flex flex-1 gap-4 p-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-0.5 text-muted-foreground">
            ...
            <p className="text-sm">{`Sleeps ${hotelInfo.details.guests} Guests`}</p>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <span className="flex items-center text-xs font-medium shrink-0 text-primary cursor-pointer">
              View More
            </span>
          </div>
        </div>
